...
final class xajaxIncludeClientScriptPlugin extends xajaxRequestPlugin
{
	...
	*/
	public function printJavascriptInclude()
	{
		...
		if (0 == count($aJsFiles)) {
			$aJsFiles[] = array($this->_getScriptFilename(base_url().'xajax_js/xajax_core.js'), 'xajax');
			
			if (true === $this->bDebug)
				$aJsFiles[] = array($this->_getScriptFilename(base_url().'xajax_js/xajax_debug.js'), 'xajax.debug');
			
			if (true === $this->bVerboseDebug)
				$aJsFiles[] = array($this->_getScriptFilename(base_url().'xajax_js/xajax_verbose.js'), 'xajax.debug.verbose');
			
			if (null !== $this->sLanguage)
				$aJsFiles[] = array($this->_getScriptFilename(base_url().'xajax_js/xajax_lang_' . $this->sLanguage . '.js'), 'xajax');
		}
